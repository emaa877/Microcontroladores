...
void config_SPI_Master(uint8_t spi_mode, uint8_t prescaler){
	...
	SPCR &= ~(3<<SPR0); //Apago el prescaler y luego enmascaro con OR
	switch(prescaler){
		case 4:
		SPCR &= ~(3<<SPR0);
		break;
...
void NRF24L01_config(uint8_t PTX_PRX, uint8_t addr, uint8_t channel){
	...
	_delay_ms(10); //solo necesita 1.5ms para pasar a modo standby (CE=LOW)
}
...
void receive_payload(void){
	CE_ON;
	_delay_ms(100); //Escucha por 100ms
	CE_OFF;
}
